...
class DemoApp(tk.Tk): #Версия через класс
    def __init__(self, *args, **kwargs):
        ...
        self.frame1.grid(columns=1, row=1)
        self.frame2 = TripleButton(self)
        self.frame2.grid(columns=2, row=1)
        self.frame3 = TripleButton(self)
        self.frame3.grid(columns=3, row=1)

        self.frame1.button1.configure(
            command = lambda: self.change_label_text("Нажатие")
        )

        self.frame2.button2.configure(
